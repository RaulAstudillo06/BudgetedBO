...
from botorch_fb.experimental.acquisition.lagrangian_ei import (
    LagrangianExpectedImprovement,
)
...
class LagrangianMultiStepLookaheadExpectedImprovement(qMultiStepLookahead):
    r"""Lagrangian Multi-Step Look-Ahead Expected Improvement (one-shot optimization)."""
    ...
    def __init__(
        ...
        r"""Lagrangian Multi-Step Look-Ahead Expected Improvement.
        ...
        # Set value functions
        # TODO: Implement MC version of Lagrangian EI to handle the case bs != 1.
        if all(bs == 1 for bs in batch_sizes):
            objective = ScalarizedObjective(weights=weights)
            valfunc_cls = [LagrangianExpectedImprovement for _ in batch_sizes]
            inner_mc_samples = None

        valfunc_argfacs = [
            lagrangian_ei_argfac(multiplier=self.multiplier) for _ in batch_sizes
        ]
...
class lagrangian_ei_argfac(Module):
    r"""Extract the best observed value from the model."""

    def __init__(self, multiplier: Union[float, Tensor]) -> None:
        super().__init__()
        self.multiplier = multiplier
