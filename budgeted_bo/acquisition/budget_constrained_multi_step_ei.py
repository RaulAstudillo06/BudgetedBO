...
from botorch_fb.experimental.acquisition.budget_constrained_ei import (
    BudgetConstrainedExpectedImprovement,
    qBudgetConstrainedExpectedImprovement,
)
...
class BudgetConstrainedMultiStepLookaheadEI(qMultiStepLookahead):
    ...
    def __init__(
        ...
        r"""Budget-Constrained Multi-Step Look-Ahead Expected Improvement.
        ...
        if use_mc_val_funcs:
            ...
            valfunc_cls = [qBudgetConstrainedExpectedImprovement for _ in batch_sizes]

            inner_mc_samples = [512 for bs in batch_sizes]
        else:
            objective = ScalarizedObjective(weights=weights)

            valfunc_cls = [BudgetConstrainedExpectedImprovement for _ in batch_sizes]

            inner_mc_samples = None

        if self.soft_plus_transform_budget:
            if self.beta is None:
                beta_adjusted = 2.0 * (len(batch_sizes) / budget)
            else:
        ...
        valfunc_argfacs = [
            budgeted_ei_argfac(
                budget=self.budget, beta=beta_adjusted, is_mc=use_mc_val_funcs
            )
